...
/**
 * The ino=put number can have very large digits. So it's better to findi divisibility by all digits, and then reuse it
 */
public class FindDigits {
    ...
    public static void main(String args[]) {
        ...
        for (int t = 0; t < numTestCases; t++) {
            ...
            boolean[] isDivisibleBy = new boolean[10];
            isDivisibleBy[0] = false;
            isDivisibleBy[1] = true;
            for (int i = 2; i < 10; i++) {
                isDivisibleBy[i] = (num % i) == 0;
            }

            long result = 0;
            while (num > 0) {
                int digit = num % 10;
                if ( isDivisibleBy[digit]) {
                    result++;
                }
                num /= 10;
            }
